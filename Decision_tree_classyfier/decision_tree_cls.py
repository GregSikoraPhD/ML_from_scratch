...
class Node:
    ...
    def __init__(self, depth: Optional[int] = None, n_samples: Optional[int] = None, feature: Optional[int] = None,
                 threshold: Optional[float] = None, information_gain: Optional[float] = None, left=None, right=None,
...
class DecisionTreeCls:
    ...
    def _grow_tree(self, X: np.ndarray, y: np.ndarray, depth: int = 0, side: str = 'root') -> Node:
        ...
        # greedy search
        best_feat, best_thresh, information_gain = self._best_criteria(X, y, feat_idxs)
        ...
        if depth == 0:
            node = Node(depth, n_samples, best_feat, best_thresh, information_gain, left, right, 'root')
            self.structure.append(node)
            return node
        else:
            node = Node(depth, n_samples, best_feat, best_thresh, information_gain, left, right, side)
            self.structure.append(node)
            return node
